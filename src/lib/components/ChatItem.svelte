<script lang="ts">
	...
	let subtitle = $derived((chatItem && chatItem.last_message) ? TelegramUtils.getTagFromMsg(chatItem.last_message) : 'Telegram is a cloud-based instant messaging service.');
...
<div onclick={onOpen} class="w-full rounded-2xl bg-[#ffffff11] h-20 flex flex-row items-center px-4" tabindex="0" role="button"  onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && onOpen()}>
